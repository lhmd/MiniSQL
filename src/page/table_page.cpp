...
TablePage::result_of_update TablePage::UpdateTuple(const Row &new_row, Row *old_row, Schema *schema, Transaction *txn,
  ...
  // If the slot number is invalid, abort.
  if (slot_num >= GetTupleCount()) {
    return result_of_update::invalid_slot_num;
  }
  uint32_t tuple_size = GetTupleSize(slot_num);
  // If the tuple is deleted, abort.
  if (IsDeleted(tuple_size)) {
    return result_of_update::deleted_tuple;
  }
  ...
  if (GetFreeSpaceRemaining() + tuple_size < serialized_size) {
    return result_of_update::not_enough_space;
  }
  ...
  return result_of_update::success;
}
