...
uint32_t CatalogMeta::GetSerializedSize() const {
  ...
  return tmp * (3 + table_meta_pages_.size() + index_meta_pages_.size()) +
         tmp * (table_meta_pages_.size() + index_meta_pages_.size());
}
...
dberr_t CatalogManager::LoadTable(const table_id_t table_id, const page_id_t page_id) {
  ...
  TableHeap *table_heap = TableHeap::Create(buffer_pool_manager_, table_meta_data->GetFirstPageId(),
                                            table_meta_data->GetSchema(), log_manager_, lock_manager_);
  TableInfo *table_info = TableInfo::Create();
