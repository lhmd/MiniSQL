...
dberr_t ExecuteEngine::ExecuteCreateDatabase(pSyntaxNode ast, ExecuteContext *context) {
  ...
  LOG(INFO) << "ExecuteCreateDatabase" << std::endl;
	...
	std::string path = "./databases" + database_name + ".db";
	dbs_.emplace(database_name, new DBStorageEngine(path));
...
dberr_t ExecuteEngine::ExecuteDropDatabase(pSyntaxNode ast, ExecuteContext *context) {
  ...
  LOG(INFO) << "ExecuteDropDatabase" << std::endl;
	...
	std::string database_name = ast->child_->val_;
	if (database_name.empty()) { return DB_FAILED; }
	if (dbs_.find(database_name) == dbs_.end()) { return DB_NOT_EXIST; }
	dbs_.erase(database_name);
	std::string path = "./databases" + database_name + ".db";
	remove(path.c_str());
	delete dbs_[database_name];
...
dberr_t ExecuteEngine::ExecuteUseDatabase(pSyntaxNode ast, ExecuteContext *context) {
  ...
  LOG(INFO) << "ExecuteUseDatabase" << std::endl;
	...
	if (dbs_.find(database_name) == dbs_.end()) { return DB_NOT_EXIST; }
	current_db_ = database_name;
	std::cout << "Database changed to " << database_name << std::endl;
	return DB_SUCCESS;
}
